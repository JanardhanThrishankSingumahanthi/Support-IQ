...
def collate_fn(batch):
    ...
    for item in batch:
        ...
        # Pad with eos/pad token (Qwen uses 151643 or pad_token_id)
        pad_id = 151643
...
def main():
    print("=" * 65)
    print("SupportIQ REAL LoRA FINE-TUNING PIPELINE")
    print("=" * 65)
    ...
    # 2. Strict Holdout Leakage Audit
    ...
    train_path = Path("backend/data/training/supportiq_train.jsonl")
    val_path = Path("backend/data/training/supportiq_val.jsonl")

    if not train_path.exists() or not val_path.exists():
        print(f"[-] Datasets missing: {train_path}, {val_path}")
        sys.exit(1)

    # Check that no holdout query exists in train
    ...
    print(f"\n[+] Loading Base Model: {model_id} in torch.float16...")
    ...
    print(f"[+] Base Model Loaded. VRAM Allocated: {mem_base:.2f} GB")
    ...
    print("\n[+] Running Pre-LoRA Baseline Inference on Base Model:")
    ...
    print(f"[+] LoRA Config: rank={lora_config.r}, alpha={lora_config.lora_alpha}, targets={lora_config.target_modules}")
    ...
    # 7. Training Loop
    print(f"\n[+] Starting Real LoRA Fine-Tuning ({num_epochs} Epochs, Batch Size={batch_size}, Grad Accum={grad_accum_steps})...")
    ...
    print("\n[+] Running Post-LoRA Fine-Tuned Inference:")
    post_lora_response = generate_response(model, tokenizer, test_q, test_ctx, device)
    print(f"    LoRA Response: \"{post_lora_response}\"")

    # 10. Record Metrics JSON
    metrics_record = {
        "model_id": model_id,
        "method": "LoRA (PEFT FP16)",
        ...
        "post_lora_response": post_lora_response,
        "output_differs": baseline_response != post_lora_response,
        "status": "PASS",
    ...
    print("\n" + "=" * 65)
    print(f"[+] SUMMARY: LoRA Training PASS [SUCCESS]")
